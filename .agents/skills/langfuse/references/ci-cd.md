...
- [ ] Confirm dataset existence and shape of the dataset items before writing code with the Langfuse CLI (see `references/cli.md`)
  ...
  - `npx langfuse-cli api dataset-items list --dataset-name <dataset name> --limit 5`
...
| Local Langfuse environment variables are not set | Set `LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, and `LANGFUSE_BASE_URL` locally before using `npx langfuse-cli` (if a tool only reads `LANGFUSE_HOST`, also `export LANGFUSE_HOST="$LANGFUSE_BASE_URL"`); do not ask the user to paste secret values into chat. |
| Workflow secrets or action inputs are wrong | Verify `LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `langfuse_base_url` (maps to `LANGFUSE_BASE_URL`), and provider secrets exist in the target repo/environment and are passed to the action step. |
